...
const parseProtocolVersion = (value) => {
  if (value === undefined || value === null || `${value}`.trim() === '') return 2;
  const parsed = Number(value);
...
/** Converts the dedicated environment contract into an explicit relay-v2 connection. */
export const createDedicatedConnection = ({ config, logger = null } = {}) => {
  ...
  const connectionId = config?.nexusApi?.connectionId ?? process.env.NEXUS_DISCORD_CONNECTION_ID;
  ...
  const relaySigner = new DiscordRelaySigner({
    ...
    keyId: context.keyId,
    currentKeyId: config?.nexusApi?.relayCurrentKeyId
      || process.env.NEXUS_DISCORD_RELAY_CURRENT_KEY_ID
      || context.keyId,
